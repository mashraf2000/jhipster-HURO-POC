...
/**
 * REST controller for managing {@link com.huro.domain.Vendor}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class VendorResource {
     ...
     */
    @GetMapping("/vendors")
    public ResponseEntity<List<Vendor>> getAllVendors(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Vendors");
        Page<Vendor> page;
        if (eagerload) {
            page = vendorRepository.findAllWithEagerRelationships(pageable);
        } else {
    ...
    public ResponseEntity<Vendor> getVendor(@PathVariable Long id) {
        ...
        Optional<Vendor> vendor = vendorRepository.findOneWithEagerRelationships(id);
